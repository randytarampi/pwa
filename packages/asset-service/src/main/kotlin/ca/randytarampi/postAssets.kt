...
suspend fun parseFilesFromMultipartEvent(event: dynamic): dynamic =
    ext._randytarampi.serverless.parseFilesFromMultipartEvent(event).await()

suspend fun generateIcons(
    inputFile: dynamic,
    outputDirectory: String,
    format: String?,
    type: String?,
): dynamic =
    ext._randytarampi.pwa_asset_generator.generateIcons(
        inputFile = inputFile,
...
suspend fun generateSplashScreens(
    inputFile: dynamic,
    outputDirectory: String,
    format: String?,
    type: String?,
): dynamic =
    ext._randytarampi.pwa_asset_generator.generateSplashScreens(
        inputFile = inputFile,
        outputDirectory = outputDirectory,
        format = format,
        type = type,
